...
public class RadioTowerChannelAssignment {
    //Write your code
     private static boolean isSafe(int node, List<List<Integer>> graph, int[] color, int c) {
        for (int adj : graph.get(node)) {
            if (color[adj] == c) return false; // adjacent node has same color
        }
        return true;
    }
    ...
    public static boolean isColorable(List<List<Integer>> graph, int[] color, int node, int m, int n) {
        //Write your code
        if (node == n) return true; // all towers are assigned

        for (int c = 1; c <= m; c++) { // try all available channels
...
## Output:
<img width="889" height="496" alt="image" src="https://github.com/user-attachments/assets/c8a56a21-d690-43ef-95be-f9a5814927f4" />
